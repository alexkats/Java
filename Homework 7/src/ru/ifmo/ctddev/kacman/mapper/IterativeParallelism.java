 ...
 * given predicate or apply function to all elements in list. If {@link info.kgeorgiy.java.advanced.mapper.ParallelMapper}
 * exists, then we use <code>map</code> function from <code>ParallelMapper</code>.
 *
 * @see ru.ifmo.ctddev.kacman.mapper.ParallelMapperImpl
...
public class IterativeParallelism implements ListIP {
    ...
    /**
     * Initializes local {@link info.kgeorgiy.java.advanced.mapper.ParallelMapper} instance with null.
     */
    ...
    /**
     * Initializes local {@link info.kgeorgiy.java.advanced.mapper.ParallelMapper}
     * instance with given <code>mapper</code>.
     * @param mapper instance of {@link info.kgeorgiy.java.advanced.mapper.ParallelMapper}
     */
    ...
    /**
     * For the given list of elements converts it to {@link String} or invokes <code>map</code> function from
     * {@link info.kgeorgiy.java.advanced.mapper.ParallelMapper} to do it.
     *
    ...
    public String concat(int threads, List<?> list) throws InterruptedException {
        ...
        if (mapper != null) {
            List<String> results =  map(threads, list, Object::toString);
            String localResult = "";
     ...
     * satisfy given predicate or invokes <code>map</code> function from
     * {@link info.kgeorgiy.java.advanced.mapper.ParallelMapper} to do it.
     *
     * @param threads number of threads
     * @param list list of elements
    ...
    public <T> List<T> filter(int threads, List<? extends T> list, Predicate<? super T> predicate) throws InterruptedException {
        ...
        if (mapper != null) {
            List<List<T>> results = mapper.map(part -> part.stream()
                    .filter(predicate)
                    .collect(Collectors.toList()), parts);
            List<T> localResult = new ArrayList<>();
            for (int i = 0; i < threads; i++) {
                localResult.addAll(results.get(i));
            }
            return localResult;
        } else {
            final Thread workers[] = new Thread[threads];
    ...
    /**
     * For the given list of elements we apply function to it and return result list or invokes
     * <code>map</code> function from
     * {@link info.kgeorgiy.java.advanced.mapper.ParallelMapper} to do it.
     *
     * @param threads number of threads
     * @param list list of elements
    ...
    public <T, U> List<U> map(int threads, List<? extends T> list, Function<? super T, ? extends U> function) throws InterruptedException {
        ...
        if (mapper != null) {
            List<List<U>> results = mapper.map(part -> part.stream().map(function).collect(Collectors.toList()), parts);
            List<U> localResult = new ArrayList<>();
            for (List<U> result : results) {
                localResult.addAll(result);
            }
            return localResult;
    ...
    /**
     * For the given list of elements it returns minimum of them or invokes <code>map</code> function from
     * {@link info.kgeorgiy.java.advanced.mapper.ParallelMapper} to do it.
     *
     * @param threads number of threads
     * @param list list of elements
    ...
    public <T> T minimum(int threads, List<? extends T> list, Comparator<? super T> comparator) throws InterruptedException {
        ...
        if (mapper != null) {
            List<? extends T> results = mapper.map(part -> Collections.min(part, comparator), parts);
            T commonResult = results.get(0);
            for (T result : results) {
                if (comparator.compare(commonResult, result) > 0) {
                    commonResult = result;
                }
            }
            return commonResult;
    ...
    /**
     * For the given list of elements it returns maximum of them or invokes <code>map</code> function from
     * {@link info.kgeorgiy.java.advanced.mapper.ParallelMapper} to do it.
     *
     * @param threads number of threads
     * @param list list of elements
    ...
    public <T> T maximum(int threads, List<? extends T> list, Comparator<? super T> comparator) throws InterruptedException{
        List<List<? extends T>> parts = parts(threads, list);
        if (mapper != null) {
            List<? extends T> results = mapper.map(part -> Collections.max(part, comparator), parts);
     ...
     * elements from it satisfy given predicate or invokes <code>map</code> function from
     * {@link info.kgeorgiy.java.advanced.mapper.ParallelMapper} to do it.
     *
     * @param threads number of threads
     * @param list list of elements
    ...
    public <T> boolean all(int threads,
        ...
        List<List<? extends T>> parts = parts(threads, list);
        if (mapper != null) {
            List<Boolean> results = mapper.map((part) -> (part.stream().allMatch(predicate)), parts);
     ...
     * element from it satisfies given predicate or invokes <code>map</code> function from
     * {@link info.kgeorgiy.java.advanced.mapper.ParallelMapper} to do it.
     *
     * @param threads number of threads
     * @param list list of elements
    ...
    public <T> boolean any(int threads,
        ...
        List<List<? extends T>> parts = parts(threads, list);
        if (mapper != null) {
            List<Boolean> results = mapper.map((part) -> (part.stream().anyMatch(predicate)), parts);
            for (Boolean res : results) {
                if (res) {
                    return true;
                }
